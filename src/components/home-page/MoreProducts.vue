<template>
    <div class="products-swiper swiper-small-screen mb-16 ">
        <div class="titl d-flex justify-space-between align-center mb-5">
            <h3 :style="moreBrandsStyling"> {{
                $t('swiper.moreBrands') }}</h3>
            <a href="#" :style="ShowMoreStyling"> {{ $t('swiper.showMore') }} </a>
        </div>


        <Swiper class="pb-9 px-5" :modules="modules" :slides-per-view="calculateSlidesPerView()" :space-between="26"
                ...
                `${item.description}`.substring(0, 57) }} </v-card-text>
                            ...
                            <div class="mt-5">
                                <v-btn @click="addToCart(item)" density="compact" class="py-2 px-12"
                                    style="letter-spacing: revert; text-transform: none;background-color: #FF9900; font-weight: 700; font-size: 15px; color: #ffffff;width: 40%; border-radius: 14px;">
